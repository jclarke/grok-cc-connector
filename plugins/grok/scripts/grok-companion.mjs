...
function printUsage() {
  console.log(
    [
      ...
      "  node scripts/grok-companion.mjs review [--wait|--background] [--base <ref>] [--scope <auto|working-tree|branch>] [focus text]",
      "  node scripts/grok-companion.mjs adversarial-review [--wait|--background] [--base <ref>] [--scope <auto|working-tree|branch>] [focus text]",
      "  node scripts/grok-companion.mjs task [--background] [--write] [--resume-last|--resume|--fresh] [--model <composer|build|grok-composer-2.5-fast|grok-build>] [--effort <none|minimal|low|medium|high|xhigh>] [prompt]",
...
async function buildSetupReport(cwd, actionsTaken = []) {
  ...
  if (!grokStatus.available) {
    nextSteps.push("Install Grok from https://x.ai/cli.");
  }
  if (grokStatus.available && !authStatus.loggedIn) {
    nextSteps.push("Run `!grok login` or set `XAI_API_KEY`, then rerun `/grok:setup`.");
  }
...
function appendReviewOutputSchema(prompt) {
...
async function executeTaskRun(request) {
  ...
  const result = await runAcpTurn(workspaceRoot, {
    ...
    alwaysApprove: Boolean(request.write),
    onProgress: request.onProgress,
  ...
  if (interrupt.attempted) {
    appendLogLine(
      ...
      interrupt.interrupted
        ? `Requested Grok session interrupt for ${sessionId}.`
        : `Grok session interrupt failed${interrupt.detail ? `: ${interrupt.detail}` : "."}`
    );
  }
